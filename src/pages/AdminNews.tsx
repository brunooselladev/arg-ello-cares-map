...
import {
  ...
  useDeleteNews,
  useNews,
  useToggleNewsVisibility,
  useUpdateNews,
} from '@/hooks/useNews';
import { NEWS_CATEGORY_LABELS, News, NewsCategory } from '@/types/database';
...
const emptyForm = {
  ...
  summary: '',
  image: '',
  category: 'nodos' as NewsCategory,
  videoUrl: '',
  date: '',
  author: '',
  tags: '',
...
const categories = Object.keys(NEWS_CATEGORY_LABELS) as NewsCategory[];
...
export default function AdminNews() {
  ...
  const handleEdit = (item: News) => {
    ...
    setForm({
      ...
      image: item.image || '',
      category: (item.category || 'nodos') as NewsCategory,
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Estas seguro de eliminar esta novedad?')) return;
  ...
  const handleToggleVisibility = async (item: News) => {
    try {
        ...
        <Card>
          <CardContent className="p-4 border-b">
            <div className="max-w-xs">
              ...
              <Select value={filterCategory} onValueChange={(value) => setFilterCategory(value as 'all' | NewsCategory)}>
          ...
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <div className="p-8 text-center text-muted-foreground">No hay novedades para el filtro seleccionado.</div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableRow key={item.id}>
                      ...
                      <TableCell>
                        <span className="px-2 py-1 rounded-full text-xs bg-secondary text-secondary-foreground">
                          {NEWS_CATEGORY_LABELS[(item.category || 'nodos') as NewsCategory]}
                        </span>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1">
                          <Button
                            ...
                            title={item.visible ? 'Ocultar en Home' : 'Mostrar en Home'}
                          >
