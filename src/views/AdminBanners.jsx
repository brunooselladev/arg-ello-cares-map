    ...
    return (<AdminLayout>
      <div className="space-y-6">
          ...
          <CardContent className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id="banner-file" type="file" accept="image/*" disabled={maxReached || createBanner.isPending} onChange={handleSelectFile}/>
              </div>

              <div className="space-y-2">
                <Label htmlFor="banner-url">O pegar URL</Label>
                <Input id="banner-url" value={imageUrlInput} onChange={(e) => setImageUrlInput(e.target.value)} placeholder="https://..." disabled={maxReached || createBanner.isPending}/>
              </div>
            </div>

            {previewSrc && (<div className="rounded-lg overflow-hidden border max-w-lg">
            ...
            {maxReached && (<p className="text-sm text-amber-600">Ya alcanzaste el limite de 3 banners. Elimina uno para continuar.</p>)}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? (<p className="text-muted-foreground">Cargando banners...</p>) : banners.length === 0 ? (<p className="text-muted-foreground">No hay banners cargados.</p>) : (<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {banners.map((banner) => (<div key={banner.id} className="rounded-lg border overflow-hidden bg-card">
                    ...
                    <div className="p-3 space-y-2">
                      ...
                      <Button variant="destructive" size="sm" onClick={() => handleDelete(banner.id)} className="w-full">
