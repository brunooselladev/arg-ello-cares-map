...
interface MapFiltersProps {
  activeFilter: MapPointType | null;
  onFilterChange: (filter: MapPointType | null) => void;
  counts: Record<MapPointType, number>;
}

const filterConfig: Record<MapPointType, { icon: typeof MapPin; colorClass: string }> = {
...
export function MapFilters({ activeFilter, onFilterChange, counts }: MapFiltersProps) {
  const filters: (MapPointType | null)[] = [null, 'nodo', 'centro_escucha', 'comunidad_practicas'];
        ...
        return (
          ...
          >
            {Icon ? (
              ...
              <span className="w-3 h-3 rounded-full bg-gradient-to-r from-nodo via-centro-escucha to-comunidad-practicas" />
            )}
