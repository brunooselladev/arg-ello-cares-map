...
export function MapFilters({ activeFilter, onFilterChange, counts }) {
    const filters = [null, 'nodo', 'centro_escucha'];
    return (<div className="flex flex-wrap gap-2 p-3 glass rounded-lg">
      {filters.map((filter) => {
            ...
            {Icon ? (<span className={cn('w-3 h-3 rounded-full', config.colorClass)}/>) : (<span className="w-3 h-3 rounded-full bg-gradient-to-r from-nodo to-centro-escucha"/>)}
