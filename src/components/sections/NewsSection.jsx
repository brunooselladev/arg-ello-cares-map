      ...
      <div className="container space-y-8">
        ...
        {isLoading ? (<div className="space-y-5">
            ...
            <div className="flex gap-4 overflow-hidden">
              {[...Array(3)].map((_, i) => (<Skeleton key={i} className="h-44 w-[280px] shrink-0"/>))}
            </div>
          </div>) : isError ? (<p className="text-center text-muted-foreground">No se pudieron cargar las novedades.</p>) : !latestItem ? (<p className="text-center text-muted-foreground">No hay novedades visibles para esta categoria.</p>) : (<>
            ...
            <div>
              ...
              {historyItems.length === 0 ? (<p className="text-sm text-muted-foreground">No hay historial adicional por ahora.</p>) : (<div className="overflow-x-auto pb-2">
                  <div className="flex gap-4 min-w-max">
                    {historyItems.map((item, index) => (<NewsCard key={item.id} news={item} index={index} compact/>))}
                    {hasNextPage && <div ref={sentinelRef} className="w-4 shrink-0" aria-hidden/>}
                  </div>
                </div>)}

              {isFetchingNextPage && (<p className="mt-2 text-sm text-muted-foreground">Cargando mas novedades...</p>)}
            </div>
          </>)}
      </div>
    </section>);
