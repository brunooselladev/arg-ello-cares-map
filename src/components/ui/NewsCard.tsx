...
export function NewsCard({ news, index = 0, compact = false }: NewsCardProps) {
  ...
  return (
    <motion.div
      ...
      className={compact ? 'w-[280px] shrink-0' : ''}
    >
      <Card className="overflow-hidden h-full hover:shadow-lg transition-shadow">
        {news.image && (
          <div className={compact ? 'relative h-36 overflow-hidden' : 'relative aspect-video overflow-hidden'}>
            <img
              src={news.image}
              alt={news.title}
        ...
        <CardContent className={compact ? 'p-4' : 'p-5'}>
          ...
          {news.summary && (
            <p className={compact ? 'text-sm text-muted-foreground line-clamp-2' : 'text-sm text-muted-foreground line-clamp-3'}>
              {news.summary}
            </p>
          )}
        </CardContent>
